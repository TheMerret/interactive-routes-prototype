...
export default function NavigationScreen({ waypoint, waypointIndex, totalWaypoints, routeTitle, onGetTask, onExit }) {
    ...
    <div style={{ background: '#f5f5f5', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '8px 24px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, height: 4, background: '#ddd', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ width: `${((waypointIndex) / totalWaypoints) * 100}%`, height: '100%', background: '#333', borderRadius: 4, transition: 'width .4s' }} />
        </div>
        <span style={{ fontSize: 13, color: '#888', whiteSpace: 'nowrap' }}>{waypointIndex}/{totalWaypoints}</span>
      </div>
      ...
      {/* Map placeholder */}
      <div style={{
        margin: '16px 16px 0', flex: 1,
        background: '#d0d0d0', borderRadius: 20,
        overflow: 'hidden', position: 'relative', minHeight: 200,
      }}>
        <div style={{
          position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center', gap: 8, color: 'rgba(0,0,0,.4)',
        }}>
      ...
      <div style={{ padding: '16px 24px 40px' }}>
        <button
          ...
          style={{
            ...
            borderRadius: 20, fontSize: 17, fontWeight: 500,
            cursor: 'pointer',
          }}
        >
          Получить задание
