...
export default function RoutesListScreen() {
  ...
  const [activeFilters, setActiveFilters] = useState({ city: [] as string[], category: [] as string[], duration: [] as string[] });

  const hasActiveFilters = Object.values(activeFilters).some(a => a.length > 0);

  const filtered = routes.filter(r => {
    if (activeFilters.city.length && !activeFilters.city.includes(r.city)) return false;
    if (activeFilters.category.length && !r.tags.some(t => activeFilters.category.includes(t))) return false;
    return true;
  });
      ...
      <div style={{ overflowY: 'auto', flex: 1, paddingBottom: 83 }}>
        ...
        <div style={{ padding: '10px 24px 0', display: 'flex', gap: 8 }}>
          ...
          <button
            ...
            style={{ width: 50, height: 42, background: hasActiveFilters ? '#333' : 'white', borderRadius: 30, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <svg width="20" height="14" viewBox="0 0 20 14" fill="none">
              <path d="M1 1h18M4 7h12M7 13h6" stroke={hasActiveFilters ? 'white' : '#333'} strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
        </div>
        ...
        {hasActiveFilters && (
          <div style={{ padding: '10px 24px 0', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[...activeFilters.city, ...activeFilters.category, ...activeFilters.duration].map(f => (
              <span key={f} style={{ background: '#333', color: 'white', borderRadius: 30, padding: '5px 12px', fontSize: 13 }}>{f}</span>
            ))}
          </div>
        )}
